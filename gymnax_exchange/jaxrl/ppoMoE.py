...
def make_train(config):
    ...
    def train(rng):
        ...
        elif config['RNN_TYPE'] == "GRU_MoE":
            network = ActorCriticMoE(env.action_space(env_params).shape[0], config=config)
            num_experts = config['num_experts']
            assert config['JOINT_ACTOR_CRITIC_NET']
            init_hstates = [ScannedRNN.initialize_carry(config["NUM_ENVS"], config["HIDDEN_SIZE"]) \
        ...
        def _update_step(runner_state, unused):
                ...
                def _update_minbatch(train_state, batch_info):
                    ...
                    def _loss_fn(params, init_hstate, traj_batch, gae, targets):
                        def debug_log(metric):
                            (update_step, info, obs_norm, action_mean, action_std) = metric
                            data = {
                        ...
                        if config['RNN_TYPE'] == "GRU_MoE":
                            init_hstate = init_hstate[0]
                            {
                                # TODO fix this, we has init_states rather than init_state
                            }
            ...
            if config["RNN_TYPE"] == "GRU":
                ...
                init_hstate = initial_hstate # TODO fix this
            else:
